...
namespace ETHotfix
{
    public static class TurntableComponentSystem
    {
        ...
        public static async Task<TurntableGoods> DarwLottery(this TurntableComponent turntableComponent, long userId, IResponse response)
        {
            ...
            {
                User user = await UserHelper.QueryUserInfo(userId);

                if (user.Jewel >= DarwLotteryConsumeJewelNum)
                {
                    UserHelper.GoodsChange(userId, GoodsId.Jewel, DarwLotteryConsumeJewelNum * -1, GoodsChangeType.DrawLottery); //改变物品给用户服
                    return turntableComponent.RandomDarwLottery(userId);
                }
                else
                {
        ...
        public static TurntableGoods RandomDarwLottery(this TurntableComponent turntableComponent, long userId)
        {
            ...
            for (int i = 0; i < turntableComponent.mTurntableGoodses.Count; i++)
            {
                ...
                if (randNum < currProbability)
                {
                    ...
                    {
                        UserHelper.GoodsChange(userId, turntableComponent.mTurntableGoodses[i].GoodsId, turntableComponent.mTurntableGoodses[i].Amount, GoodsChangeType.DrawLottery); //改变物品给用户服
                    }
        ...
        // 记录中奖信息
        public static async void RecordWinPrizeInfo(this TurntableComponent turntableComponent, long userId, TurntableGoods goods)
        {
