...
namespace ETHotfix
{
    ...
    public static class MatchRoomFactory
    {
        ...
        // 创建房卡匹房间
        public static MatchRoom Create(RepeatedField<int> roomConfigLists, long gameEntryId, int roomId, int friendsCircleId, long userJewelNum, IResponse iResponse)
        {
            int needJeweNumCount = 0;
            int number = 0;
            bool isAADeductJewel = false;
            //效验房间配置正不正确 不正确的值会修改为默认值  会赋值 需要的钻石数 和房间配置人数
            if (!RoomConfigIntended.IntendedRoomConfigParameter(roomConfigLists, gameEntryId, ref needJeweNumCount, ref number, ref isAADeductJewel))
            {
            ...
            MatchRoomConfig roomConfig = MatchRoomConfigFactory.Create(number, roomId, gameEntryId, roomConfigLists);
        ...
        //创建随机匹配房间
        public static MatchRoom CreateRandomMatchRoom(long gameEntryId, int roomId, RepeatedField<MatchPlayerInfo> matchPlayerInfos, MatchRoomConfig matchRoomConfig)
        {
            try
            {
                MatchRoom matchRoom = ComponentFactory.Create<MatchRoom>();
