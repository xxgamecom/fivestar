...
namespace ETHotfix
{
    ...
    public class S2U_UserGetGoodsHandler: AMRpcHandler<S2U_UserGetGoods, U2S_UserGetGoods>
    {
        protected override async void Run(Session session, S2U_UserGetGoods message, Action<U2S_UserGetGoods> reply)
        {
            ...
            try
            {
                response.user = await Game.Scene.GetComponent<UserManager>()
                                          .UserGetGoods(message.UserId, message.GetGoodsList, message.GoodsChangeType, message.isShowHintPanel);
                reply(response);
            }
            catch (Exception e)
            {
