...
namespace ETHotfix
{
    ...
    public class C2U_QueryUserInfoHandler: AMRpcHandler<C2U_QueryUserInfo, U2C_QueryUserInfo>
    {
        protected override async void Run(Session session, C2U_QueryUserInfo message, Action<U2C_QueryUserInfo> reply)
        {
            ...
            try
            {
                User user = await UserManager.Ins.Query(message.QueryUserId);
                if (user == null)
                {
                ...
                List<AccountInfo> accountInfos =
                        await UserManager.Ins.dbProxyComponent.Query<AccountInfo>(accountInfo => accountInfo.UserId == message.QueryUserId);
                ...
                //查询代理等级
                List<AgecyInfo> agecyInfos =
                        await UserManager.Ins.dbProxyComponent.Query<AgecyInfo>(agecyInfo => agecyInfo.UserId == message.QueryUserId);
