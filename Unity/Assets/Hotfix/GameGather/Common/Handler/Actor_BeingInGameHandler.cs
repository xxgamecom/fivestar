    ...
    public class Actor_BeingInGameHandler: AMHandler<Actor_BeingInGame>
    {
        protected override void Run(ETModel.Session session, Actor_BeingInGame message)
        {
            ...
            else
            {
                ...
                {
                    CardFiveStarEntry.JoinRoom(SdkCall.OpenAppUrl);
                }
            }
        ...
        {
            var m2CGetReconnection = (M2C_GetReconnectionRoomInfo)await SessionComponent.Instance.Call(new C2M_GetReconnectionRoomInfo()); //请求断线重连数据
            if (m2CGetReconnection.IsGameBeing)
            {
                //这个roomifno 信息 不为空 表示在等待状态
                if (m2CGetReconnection.RoomInfos != null)
                {
                    CardFiveStarEntry.RoomCardEnterRoom(m2CGetReconnection.RoomInfos); //只有卡五星 而且在等待状态重连 只有可能是房卡模式
                }
            }
            else
            {
